    ...
    <div v-if="totalItemsInTheCart > 0">
      <ul class="cart-added-items">
        <li v-for="(item, index) in cart" :key="item._id + '-' + index">
          <img
            :src="`${ELASTIC_BEANSTALK_API_URL}/images/` + item.image"
            :alt="item.subject"
            width="50"
            height="50"
          />
          <p>
            ...
            <span
              >- Total Price: <strong>£{{ Math.round(item.totalPrice) }}</strong>
            </span>
          </p>
      ...
      <button
        @click="checkout(checkoutForm)"
        :disabled="cart.length === 0 || !isInputValid"
      >
        Checkout
      </button>
    </div>
...
export default {
  ...
  computed: {
    ...
    isInputValid() {
      ...
      const isNameValid = /^[A-Za-z]+$/.test(name.trim()) && name.trim().length > 1
